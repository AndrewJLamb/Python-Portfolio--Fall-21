...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug 29 11:10:14 2021
...
###                      EWOP STUDIOS                                      ###
###    DATA ANALYSIS on : {Client Name}, SPOTIFY for ARTISTS RAW DATA       ###
###           BY: ANDREW LAMB, LAURENCE HICKS                              ###
###    (QUESTIONS OR CONCERNS? Email: andrew@ewopstudios.com)              ###
                                                           
#Importing 'timelines(15) 06282021thru08282021.csv':
...
#date; followers; listeners; streams for 
    #June 28th, 2021 thru August 28th, 2021;
...
tl = pd.read_excel('/Users/'
                     ...
                     '[SPOTIFY_py]/'
                     '(.xlsx, .csv) files/'
                     'timelines(15) 06282021thru08282021.xlsx')
#Use pd.set_option to print all rows and columns;
pd.set_option('display.max_rows', None)
pd.set_option('display.max_columns', None)
print(f'tl.head(): \n{tl.head()}\n')     #Checking the first and last few rows                            
print(f'tl.tail(): \n{tl.tail()}\n')     #of timelines(15) 06282021thru08282021


K = len(tl)

tl['month'] = pd.DatetimeIndex(tl['date']).month
tl['day'] = pd.DatetimeIndex(tl['date']).day


#Working out demog. as rough time-series estimate 
#[Demog.] gender %'s (rough, 08/28/2021 collection date)
...
tl['lg2'] = (tl['listeners'] 
                 * 0.09).astype('int32')        #Female= ~9%  
                                                #Previously- Female= ~13%;

#[Demog.] Non-binary identifying listeners
# (as a % of total listeners)
tl['lg3'] = tl['listeners'].astype('int32') #(as a % of total listeners);
tl['lg3'] = (tl['listeners'] 
                  * 0.04).astype('int32')       #NB+NS = ~4%  
                                                #Previously-NB= ~2%; 
                                                #Added the 2% recorded as non-specified;
...
tl['la2'] = (tl['listeners'] 
                   * 0.36).astype('int32') 
                                            ...
                                            #Previously- ~36%
                                            #Listeners between 18 and 22= ~42%;
...
tl['la3'] = (tl['listeners'] 
                   * 0.19).astype('int32')
                                            ...
                                            #Previously- ~19%
                                            #Listeners between 23 and 27= ~20%;
...
tl['la4'] = (tl['listeners']
                   * 0.17).astype('int32') 
                                            ...
                                            #Previously- ~17%
                                            #Listeners between 28 and 34= ~13%;
...
#Exporting new 'tl' (generated above) printed output into the original Excel file;
writer_obj = pd.ExcelWriter('Timelines(15).xlsx',
                            engine='xlsxwriter')
...
ax.set_title('{Client Name}: '
             'Total Spotify Listeners 06/28/2021- 08/28/2021')  #Title
