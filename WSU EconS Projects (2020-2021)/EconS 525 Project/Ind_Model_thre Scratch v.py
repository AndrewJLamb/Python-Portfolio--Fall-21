...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 14 13:07:23 2021
...
th.replace([np.inf, -np.inf], np.nan, inplace=True)
th = th.fillna(0)
...
dCountry = pd.get_dummies(th.id, prefix='d_id')
th = pd.concat([th,dCountry], axis=1)

#Destination
#Creating dummy variable for 'dest':
ddest = pd.get_dummies(th.dest, prefix='ddest')
th = pd.concat([th,ddest], axis=1)

#HS Category
#Creating dummy variable for 'hscode':
dhts = pd.get_dummies(th.hscode, prefix='dhts')
th = pd.concat([th,dhts], axis=1)
...
th['tw1'] = ((th['date'] >='2018-03') 
                             & (th['date'] <='2021-02')).astype(int)
#Creating dummy variable for 'trade war':
# 'tw1' = Obs. in Feb. 2018;
#Noted on timeline to be a time period of interest for 
    #U.S.-China trade negotiations, retaliation;
th['tw2'] = ((th['date'] >='2018-01') 
                             & (th['date'] <='2018-02')).astype(int)
#COVID-19 Pandemic
#Creating a dummy variable for 'covid':
# 'covid' = Obs. in Mar. 2019 thru Feb. 2021;
#Noted to be a time period of interest;
th['covid'] = ((th['date'] >='2019-02') 
                             & (th['date'] <='2021-02')).astype(int)
...
print('Threads Group Estimation:')
print(f'table: \n{table}\n')
