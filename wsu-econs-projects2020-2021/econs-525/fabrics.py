...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 14 13:07:28 2021
...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 13 16:05:15 2021
...
dCountry = pd.get_dummies(fa.id, prefix='d_id')
fa = pd.concat([fa,dCountry], axis=1)

#Destination
#Creating dummy variable for 'dest':
ddest = pd.get_dummies(fa.dest, prefix='ddest')
fa = pd.concat([fa,ddest], axis=1)

#HS Category
#Creating dummy variable for 'hscode':
dhts = pd.get_dummies(fa.hscode, prefix='dhts')
fa = pd.concat([fa,dhts], axis=1)
...
fa['tw1'] = ((fa['date'] >='2018-03') 
                             & (fa['date'] <='2021-02')).astype(int)
#Creating dummy variable for 'trade war':
# 'tw1' = Obs. in Feb. 2018;
#Noted on timeline to be a time period of interest for 
    #U.S.-China trade negotiations, retaliation;
fa['tw2'] = ((fa['date'] >='2018-01') 
                             & (fa['date'] <='2018-02')).astype(int)

#COVID-19 Pandemic
#Creating a dummy variable for 'covid':
# 'covid' = Obs. in Mar. 2019 thru Feb. 2021;
#Noted to be a time period of interest;
fa['covid'] = ((fa['date'] >='2019-02') 
                             & (fa['date'] <='2021-02')).astype(int)
...
print('Fabrics Group Estimation:')
print(f'table: \n{table}\n')
